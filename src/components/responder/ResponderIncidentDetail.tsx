...
export const ResponderIncidentDetail: React.FC = () => {
  ...
  if (!incident) {
    return (
      <div className="space-y-4">
        <Link to="/responder/incidents" className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-white">
          <ArrowLeft className="w-3.5 h-3.5" /> Back to Responder Incidents
        </Link>
  ...
  const assignedTeam = teams.find(t => t.currentAssignmentIncidentId === incidentId);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-900/90 border border-gray-800 rounded-2xl p-5 shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="bg-gray-950/60 p-3 rounded-xl border border-gray-800">
            <span className="text-[10px] text-gray-400 block">PEOPLE</span>
            <span className="text-sm font-bold text-white">~{incident.estimatedPeopleAffected || 'N/A'}</span>
          </div>
          <div className="bg-gray-950/60 p-3 rounded-xl border border-gray-800">
            <span className="text-[10px] text-gray-400 block">ASSIGNED TEAM</span>
            <span className="text-sm font-bold text-white">{assignedTeam?.name || 'Unassigned'}</span>
          </div>
          <div className="bg-gray-950/60 p-3 rounded-xl border border-gray-800">
